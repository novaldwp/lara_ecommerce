            ...
            <div class="col-12">
                <div class="card mb-0">
                    ...
                    <ul class="nav nav-pills">
                        <li class="nav-item">
                        <a class="nav-link active" href="#"> {{ getStatus(2) }} <span class="badge badge-white" name="all">{{ $countStatus['all'] }}</span></a>
                        </li>
                        <li class="nav-item">
                        <a class="nav-link" href="javascript:void(0);" data-filter="1">{{ getStatus(3) }} <span class="badge badge-primary" name="featured">{{ $countStatus['featured'] }}</span></a>
                        </li>
                        <li class="nav-item">
                        <a class="nav-link" href="javascript:void(0);" data-filter="2">{{ getStatus(4) }} <span class="badge badge-primary" name="featured">{{ $countStatus['nonfeatured'] }}</span></a>
                        </li>
                        <li class="nav-item">
...
$(document).ready(function() {
    ...
    $(document).on('click', '#deleteButton', function(e) {
        ...
        swal({
            ...
            text: "Produk ini akan di non-aktifkan",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            })
    ...
    $(document).on('click', '#restoreButton', function(e) {
        ...
        swal({
            title: "Apakah Kamu Yakin?",
            text: "Produk ini akan diaktifkan",
            icon: "warning",
            buttons: true,
            dangerMode: true,
            })
            ...
            if (willDelete) {
                $.ajax({
                    url: 'products/restore/'+product_id,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(res)
                    {
